<!-- src/core/views/follow-up/components/follow-up-widget.vue -->
<!-- Kompaktes Dashboard-Widget für Wiedervorlagen -->
<template>
    <v-card class="follow-up-widget" variant="outlined">
        <!-- Header -->
        <v-card-title class="d-flex align-center pa-3 bg-amber-lighten-5">
            ...
            <v-btn
                ...
                variant="text"
                :to="{ name: 'follow-up' }"
                :title="t('FollowUpWidget.openAll')"
            >
        ...
        <template v-else>
            ...
            <v-list density="compact" class="widget-list">
                ...
                <v-list-item v-if="displayItems.length === 0">
                    <v-list-item-title class="text-center text-grey py-4">
                        ...
                        <div>{{ t('FollowUpWidget.noItems') }}</div>
                    </v-list-item-title>
                </v-list-item>
                ...
                >
                    ...
                    <template #append>
                        <v-btn
                            ...
                            :title="t('FollowUpWidget.markDone')"
                        >
            ...
            <v-card-actions v-if="totalItems > maxDisplay" class="pa-2">
                <v-btn
                    ...
                    :to="{ name: 'follow-up' }"
                >
                    {{ t('FollowUpWidget.showAll', { count: totalItems }) }}
        ...
        <v-fab
            ...
            :title="t('FollowUpWidget.quickAdd')"
        />
...
import FollowUpDialog from './follow-up-dialog.vue';
...
async function loadDashboard() {
    loading.value = true;
    try {
        const result = await store.apiCall('followUp', 'getDashboard', {});
        if (result.success) {
            dashboardData.value = result;
        }
    } catch (error) {
        console.error('Error loading follow-up dashboard:', error);
    } finally {
        loading.value = false;
    }
}
...
async function markDone(item) {
    try {
        await store.apiCall('followUp', 'markDone', { id: item.id });
        loadDashboard();
        emit('refresh');
    } catch (error) {
...
function openItem(item) {
    router.push({ name: 'follow-up', query: { edit: item.id } });
}
...
async function onSave(data) {
    try {
        await store.apiCall('followUp', 'create', data);
...
function formatDate(dateStr) {
    ...
    if (date.toDateString() === today.toDateString()) {
        return t('FollowUpWidget.today');
    }

    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    if (date.toDateString() === tomorrow.toDateString()) {
        return t('FollowUpWidget.tomorrow');
    }

    const diffDays = Math.ceil((date - today) / (1000 * 60 * 60 * 24));
    if (diffDays < 0) {
        return t('FollowUpWidget.daysAgo', { days: Math.abs(diffDays) });
    }
